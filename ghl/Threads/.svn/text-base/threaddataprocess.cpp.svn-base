    ...
    QObject(parent)
{
    ...
    connect(this, &ThreadDataProcess::sigGetData, dataProcess, &DataProcess::slotDataQuery);
    connect(dataProcess, &DataProcess::sigResultReady, this, &ThreadDataProcess::slotHandleResults);
 ...
 * 	  无
 */
void ThreadDataProcess::slotHandleResults()
{
    //根据配置决定是否进行MATH运算
    emit sigDataProcessFinished();
}
...
void MathProcess::slotMathProcess()
{
    ...
    for (int i=0; i<MathExpAnalysis::m_mathExplist.count(); ++i){
        ...
        if (pMathExpAnalysis->m_tlog_type) {
            selfNum = (Channel_Type_Math<<CHANNEL_TYPE_BITOFF) | i;
